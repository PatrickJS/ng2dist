...
import {If,
  ...
  Switch,
  SwitchWhen,
  SwitchDefault} from 'angular2/directives';
...
class BaseLineLargetableComponent {
  constructor(element, benchmarkType, rows, columns) {
    assert.argumentTypes(element, assert.type.any, benchmarkType, assert.type.any, rows, assert.type.number, columns, assert.type.number);
...
class LargetableComponent {
  constructor(benchmarkType, rows, columns) {
    this.benchmarkType = benchmarkType;
    this.rows = rows;
    this.columns = columns;
  }
